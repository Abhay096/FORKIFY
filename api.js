...
let add_recepie = document.querySelector('#add_recipe');
...
function bookmark_content() {
    ...
    if (i == arr.length) {
        let bookmark = `<div id="t${temp}" data-recipe-id="${data1.data.recipes[global_recepie_id].id}" style="display: flex;align-items: center; margin: 20px 0px 20px 0px; background-color:white; margin-left:1em; margin-right:1em">
...
function remove(element) {
    ...
    let recipeId = t.getAttribute("data-recipe-id");
    t.parentNode.removeChild(t);
    let index = arr.indexOf(recipeId);
    if (index > -1) {
        arr.splice(index, 1);
    }
}
